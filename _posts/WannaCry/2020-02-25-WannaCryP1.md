...
>> note: we will extract all process vads, so it will be easy for us to search from some information as doing this process manually will be hard, but for checking processes heaps we can extract the heaps one by one. 
...
[![47](/assets/images/WannaCry/p1/i47.png)](/assets/images/WannaCry/p1/i47.png)

and those files look to be will hold important data. 
...
let's find out if it used the cmd to do any other function. 

```bash 
strings vadDump2464/* | grep -i 'cmd.exe'
```

[![42](/assets/images/WannaCry/p1/i42.png)](/assets/images/WannaCry/p1/i42.png)

it used cmd to start @WanaDecryptor@.exe

lets search again for all exe in this process 
